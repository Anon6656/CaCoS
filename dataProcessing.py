...
class ProcessDataset(Dataset):
    def __init__(self, graph, label_file, dataset = 'Cora', decom_type = 'core', precomp_feat = 'yes', **kwargs):
        ...
        self.precomp_feat = precomp_feat
        self.seed = 1
    ...
    def cohesive_subgraph_extraction(self, edge_index, edge_attr, x):
        ...
        for attr_value in unique_edge_attrs:
            ...
            # # Create CORRECT mapping: {subgraph_idx: original_idx}
            # original_node_indices = {
            #     i: node.item() 
            #     for i, node in enumerate(subgraph_node_indices)
            # }
        ...
        if self.precomp_feat == 'no':
            ...
            if self.decom_type == 'truss':
                G_main, trussness_dict = \
                    self.get_all_edges_trussness_and_decomposition(G, t_len = 0)
            elif self.decom_type == 'core':
                G_main, trussness_dict = \
                    self.get_all_edges_coreness_and_decomposition(G, t_len = 0)
            ...
            data = Planetoid(root='/tmp/'+str(self.dataset), name=self.dataset)
            self.PyG_data_graph = data[0]
            print(self.PyG_data_graph)
  
            G = pyg_utils.to_networkx(self.PyG_data_graph)
            ...
            subgraph_tensor = self.cohesive_subgraph_extraction(self.PyG_data_graph.edge_index, self.PyG_data_graph.edge_attr, self.PyG_data_graph.x)
            self.PyG_data_graph.subgraphs = list(subgraph_tensor.values())
            self.PyG_data_graph.num_classes = data.num_classes
